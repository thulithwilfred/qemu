...
static bool nvme_init_pci(NvmeCtrl *n, PCIDevice *pci_dev, Error **errp)
{
    ...
    if (pci_dev->spdm_port) {
        /* Start DOE cfg space after sriov if it's used */
        doe_offset = n->params.sriov_max_vfs ? PCI_CONFIG_SPACE_SIZE + PCI_ARI_SIZEOF : PCI_CONFIG_SPACE_SIZE;
        pcie_doe_init(pci_dev, &pci_dev->doe_spdm, doe_offset, doe_spdm_prot, true, 0);
        pci_dev->doe_spdm.socket = spdm_sock_init(pci_dev->spdm_port, errp);
